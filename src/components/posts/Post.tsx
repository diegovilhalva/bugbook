...
function MediaPreview({ media }: MediaPreviewProps) {
  if (media.type === "IMAGE") {
    return <Image src={media.url} alt="Imagem" width={500} height={500} className="mx-auto size-fit max-h-[30rem] rounded-2xl" />
  }
  if (media.type === "VIDEO") {
    return <div>
  ...
  return <p className="text-destructive">Formato de media não suportado</p>
}
...
function CommentButton({ post, onClick }: CommentButtonProps) {
  return <button onClick={onClick} className="flex items-center gap-2">
    ...
    {post._count.comments === 1 ?   <span className="hidden sm:inline">Comentário</span> :  <span className="hidden sm:inline">Comentários</span>}
    </span>
  </button>
}